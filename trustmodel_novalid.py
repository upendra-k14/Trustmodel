...
class BasicModel(object):
    ...
    def get_evidence(self):
        ...
        st_succ_indices = tf.nn.embedding_lookup(self.g_indices, self.s_nodes)
        end_succ_indices = tf.nn.embedding_lookup(self.g_indices, self.s_nodes+1)
    ...
    def ranking_margin_objective(self):
        ...
        phi1 = tf.multiply(
            self.alpha1,
            tf.reduce_sum(tf.multiply(self.H,tf.norm(self.src_emb-self.target_emb, axis=0))))
        phi2 = tf.multiply(
            self.alpha2,
            tf.reduce_sum(tf.multiply(self.I,tf.norm(self.src_emb-self.target_emb, axis=0))))
    ...
    def train(self, g_indices, g_data, walks, np_pairs, logger, global_H, valid_pairs, test_pairs, orig_set, max_iter = 600):
        """
