...
class BasicModel(object):
    ...
    def __init__(self, n_nodes, n_dimension, batch_size=1024,
        learning_rate=0.0001, alpha=[0.1, 0.1, 0.1], margin=1.0):
    ...
    def ranking_margin_objective(self):
        ...
        phi1 = tf.multiply(
            self.alpha1,
            tf.reduce_sum(tf.multiply(self.H,tf.abs(pos_output-1))))
        phi2 = tf.multiply(
            self.alpha2,
            tf.reduce_sum(tf.multiply(self.I,tf.abs(pos_output-1))))
    ...
    def train(self, g_indices, g_data, walks, np_pairs, logger, global_H, valid_pairs, test_pairs, orig_set, max_iter = 2000):
        """
        ...
        average_acc = 0.0
        iter_list = [100*i for i in range(1,21)]
        prev_acc = 0.01
        curr_iter = 1
