...
class BasicModel(object):
    ...
    def train(self, g_indices, g_data, walks, np_pairs, logger, global_H, valid_pairs, test_pairs, orig_set, max_iter = 1500):
        """
        Training and preparation of data
        """

        # convert random walks to node pairs
        pairs = np.array(np_pairs)
        np.random.shuffle(pairs)
        pairs_set = set(map(tuple,np_pairs.tolist()))
